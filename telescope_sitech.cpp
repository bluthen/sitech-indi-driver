...
//    sprintf(myLogStg,"inHandShk.B4SetupVars. nbytesread=%d rcr=%d rs=%s", nbytes_read,rcr,RcvBuf);DansLog(myLogStg);
    ...
    sprintf(myLogStg, "inHandShkAfterSetupVars. RA=%f rcvBuf=%s", currentRA, RcvBuf);
    DansLog(myLogStg);
...
//        sprintf(myLogStg,"GSFS ErrRead nbytsrd=%d rc=%drcr=%d RcvStrg=%s",nbytes_read,rc,rcr,RcvBuf);DansLog((char*)myLogStg);
        char stsg[777];
        sprintf(stsg, "Error readingg from SiTechExe TCP server. Sent=%s nByts=%d errNum=%d", SendBuf, nbytes_read,
                rcr);
...
*/
    ...
    if (PrintBools && LastScopeStt != ScopeStt) {
        sprintf(myLogStg,
                ...
                ScopeStt, IsInitialized, IsTracking, IsSlewing, IsParking, IsParked, IsLookingEast, IsInBlinky,
                IsCommunicatingWithController);
...
bool ScopeSiTech::Sync(double ra, double dec) {
    char cStr[1164];
    sprintf(cStr, "Sync %f %f 0", ra,
            dec);//the zero is call up the sitech init window.  1 = offset instantaneous, 2 = load calibration instantaneos
...
bool ScopeSiTech::Park() {
    SetUpVarsFromReturnString(GetStringFromSerial((char *) "Park 0"),
                              true);//the zero is regular park, could do 1 or 2 as well.
